...
public class Explosion : MonoBehaviour
{
	...
	[SerializeField] private bool playerExplosion = false;
	...
	IEnumerator Start()
	{
			...
			foreach (Collider col in cols)
			{
				...
				if (!playerExplosion)
				{
					...
					if (colObj.tag == "Player")
					{
						...
						if (colObj.TryGetComponent(out health) == false)//ない場合
							Debug.LogError("プレイヤーのhealthが見つかりませんでした");

						else//ある場合
						{
							health.ChangeHealth(-damage);//ダメージを与える
						}
					}
				}
		...
		foreach (Collider col in cols)
		{
			...
			if (GameObject.Find("Player") != null)//プレイヤーが存在する場合
			{
