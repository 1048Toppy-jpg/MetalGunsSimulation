...
public class Weapon : MonoBehaviour
{
	...
	private OriginalWeaponData weaponData = null;
	...
	void Start()
	{
		...
		if (audioManager.TryGetComponent(out audioSource) == false)
		{
		...
		//
		if (gameObject.TryGetComponent(out weaponData) == false)
		{
			Debug.LogError("WeaponDataが見つかりません");
		}
	...
	void Update()
	{
		...
		//	Reload();
		if (reloadAutomatically && !weaponData.CanFire())
			Reload();
	...
	void OnGUI()
	{
		...
		//	
		//		GUI.Label(new Rect(170, Screen.height - 55, 100, 50), "" + currentAmmo + " / " + ammoCapacity);
		//		
		//    }
	...
	void Fire()
	{
		...
		for (int i = 0; i < shotPerRound; i++)
		{
			...
			if (Physics.Raycast(ray, out hit, range))
			{
				...
				var originalPower = weaponData.GetPower();

				//追加(敵のダメージ処理)********
				var colObj = hit.collider.gameObject;
				if (colObj.tag == "Collider")
				{
					var hitObj = colObj.transform.parent;
					if (hitObj.tag == "Enemy")
					{
						EnemyController enemycontroller = null;
						if (hitObj.TryGetComponent(out enemycontroller))
						{
							//Debug.Log(colObj.name + "に当たりました");
							enemycontroller.HitBullet(originalPower, hit.point);
						}
					}
					else if (hitObj.tag == "Boss")
					{
						BossController enemycontroller = null;
						if (hitObj.TryGetComponent(out enemycontroller))
						{
							//Debug.Log(colObj.name + "に当たりました");
							enemycontroller.HitBullet(originalPower, hit.point);
					...
					else if (hitObj.tag == "Collider")
					{
						var hitObjTwo = hitObj.transform.parent;
						if (hitObjTwo.tag == "Enemy")
						{
							EnemyController enemycontroller = null;
							if (hitObjTwo.TryGetComponent(out enemycontroller))
							{
								enemycontroller.HitBullet(originalPower, hit.point);
							}
						}
						else if (hitObjTwo.tag == "Boss")
						{
							BossController enemycontroller = null;
							if (hitObj.TryGetComponent(out enemycontroller))
							{
				...
				else if (colObj.tag == "Target")//的の場合
				{
					Target targetObject;//的情報
					if (colObj.TryGetComponent(out targetObject) == false)
						Debug.LogError("targetObjectが見つかりませんでした");
					else
					{
						targetObject.Hit(hit.point);//ヒット
				...
				if (hit.collider.gameObject.tag == "NoTex")
				{
					exception = true;
					break;
				}
